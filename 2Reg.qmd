...
make_plot <- function(var_name) {
  ...
  # линии только для complete pairs по этой переменной
  df_line <- df_raw %>%
    group_by(pat_record_id) %>%
...
#| label: facet_wrap_plots_m12h_0h_48h_72h
#| cache: true
#| warning: false
#| message: false
...
df_raw <- plot_long %>% drop_na(value)
...
p <- ggplot() +
  ...
  geom_line(
    data = df_line,
    aes(x = timepoint, y = value, group = interaction(variable, pat_record_id)),
    alpha = 0.25
  ) +
  geom_point(
    data = df_raw,
